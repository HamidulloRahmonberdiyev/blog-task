<x-layouts.admin.app>
    ...
    <x-slot:title>Edit / {{ $article->title }}</x-slot:title>
    ...
    <div class="flex items-center justify-center p-12">
        <div class="mx-auto w-full max-w-[550px]">
          ...
          >@csrf @method('PUT')
            <div class="mb-5">
              <label for="title" class="mb-3 block text-base font-medium text-indigo-500">
                Title
              </label>
              <input
                type="text"
            ...
            <div class="mb-5">
              <label for="name" class="mb-3 block text-base font-medium text-indigo-500">
                Short text
              </label>
              <textarea
                name="short_text"
                placeholder="Short text"
                class="w-full rounded-md border border-[#e0e0e0] bg-black py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" rows="4" required>{{ $article->short_text }}</textarea>
            </div>
            ...
            <div class="mb-5 w-full bg-gray-50 rounded-lg border border-gray-200 dark:bg-gray-700 dark:border-gray-600">
              ...
              <div class="py-2 px-4 bg-white rounded-b-lg dark:bg-gray-800">
                  ...
                  <textarea id="editor" name="body" rows="8" class="block px-0 w-full text-sm text-gray-800 bg-white border-0 dark:bg-gray-800 focus:ring-0 dark:text-white dark:placeholder-gray-400" placeholder="Write an article..." required>{{ $article->body }}</textarea>
              </div>
            </div>
            ...
            <div class="mb-5">
                ...
                <select
                  name="tags[]"
                  class="w-full rounded-md border border-[#e0e0e0] bg-black py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" style="height:100px" multiple required>
                  @foreach ($tags as $tag)
            ...
            <div class="mb-8">
              <input type="file" name="photo" id="file" class="sr-only" />
              <label
                for="file"
                class="relative flex min-h-[200px] items-center justify-center rounded-md border border-dashed border-[#e0e0e0] p-12 text-center">
                <div>
                  <span class="mb-2 block text-xl font-semibold text-indigo-500">
                    Photo files here
                  </span>
                  ...
                  <span
                    class="inline-flex rounded border border-[#e0e0e0] py-2 px-7 text-base font-medium text-indigo-500">
                    Browse
                  </span>
                </div>
              </label>
