<x-layouts.admin.app>
    ...
    <div id="last-users">
        <h1 class="font-bold py-4 uppercase">Users 
            <a type="submit" href="{{ route('users.create') }}" class="rounded-xl ml-5 border-2 border-green-500 px-5 py-1 text-base mb-3 font-medium text-green-500 transition duration-200 hover:bg-green-600/5 active:bg-green-700/5">
                Create user
            </a></h1>
        <div class="overflow-x-scroll">
            <table class="w-full whitespace-nowrap">
                ...
                <tr class="border-b border-gray-700">
                    ...
                    <td class="py-3 px-2 text-indigo-400 uppercase font-bold">{{ $user->role->name }}</td>
                    <td class="py-3 px-2">{{ $user->created_at->format('d-m-Y') }}</td>
                    <td class="py-3 px-2">
                        <div class="inline-flex items-center space-x-3">
                            <a href="{{ route('users.edit', $user->id) }}" title="Edit" class="hover:text-white text-yellow-600"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
